 ...
 * registered on load using {@link RPGData#registerStoredVariables(IStoredVariable...)} in order for NBT storage
...
public interface IStoredVariable<T> extends IVariable<T> {
	...
	class StoredVariable<T, E extends NBTBase> implements IStoredVariable<T> {
		...
		/**
		 * Adds synchronisation to this variable, meaning it will be sent to clients whenever {@link RPGData#sync()}
